...
# Every path in the experiments is derived from these four, so moving the code only requires
# keeping src/experiments/ two levels below the repo root, or setting STEERING_RAG_ROOT.
REPO_ROOT = Path(os.environ.get("STEERING_RAG_ROOT", Path(__file__).resolve().parents[2]))
DATA_DIR = REPO_ROOT / "data"
NORMALIZED_DIR = DATA_DIR / "normalized_dataset"
