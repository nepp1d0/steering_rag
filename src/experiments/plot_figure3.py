...
OUT_PATH = RESULTS_DIR / "figures" / "figure_3_reranking.pdf"
...
MARKERS = {"conflictqa": "o", "nq_swap": "^"}
LINESTYLES = {"conflictqa": "-", "nq_swap": "--"}
...
def parse_parts(path: Path):
    """Assumed layout: model/eval/direction/normalize/seed_N/procedure/layer_M/results.jsonl"""
    p = path.relative_to(TOP_DIR).parts
    if len(p) < 8:
        return None
    return {"model": p[0], "eval": p[1], "direction": p[2],
            "normalize": p[3], "seed": p[4], "procedure": p[5], "layer": p[6]}
...
def collect_groups() -> dict[tuple, list[Path]]:
    ...
    for f in sorted(TOP_DIR.rglob("results.jsonl")):
        ...
        if meta["normalize"] != NORMALIZE or meta["procedure"] != PROCEDURE:
            continue
        if IN_DOMAIN_ONLY and meta["eval"] != meta["direction"]:
            continue
