"""
Step 1b - Direction identification via diff-in-means on mixtures of datasets.

Same procedure as `direction_identification.py`, but the train splits of several
datasets are pooled before computing the direction.

The "context_only" procedure is run at both positions:
  * last_pos:   last token of the chunk.
  * entity_pos: last token of the first occurrence of the answer entity inside the
    chunk (per-dataset locators shared with `direction_identification.py`; texts
    whose entity cannot be located contribute to last_pos only).

Every dataset is subsampled to the same number of texts per side (the min over all
datasets and both sides, = 987 at seed 42) so that each contributes equally to the
means and all combos are directly comparable. Activations are extracted once per
dataset per layer and reused across combos.

Outputs:
    results/mixed_directions/<model>/<combo>/seed_<S>/context_only/layer_<L>/<position>/{direction.pt,meta.json}
...
DATASETS = ["conflictqa", "nq_swap", "longfact"]
POSITIONS = ["last_pos", "entity_pos"]

# Singles, pairs, and the full mixture. All are computed from the same cached stacks.
...
def load_side_data(dataset: str, seed: int) -> Dict[str, List[Tuple[str, Optional[Tuple[int, int]]]]]:
    """{"pos"/"neg": [(text, entity_char_span_or_None)]} from the train split.

    diff-in-means needs two sets of activations, not paired rows, so the sides are
    collected independently. longfact rows fill only one side and contribute only there.
    """
    samples = load_normalized(dataset, seed)["train"]
    qa_spans = compute_conflictqa_qa_spans(samples) if dataset == "conflictqa" else None
    return {side: resolve_side_spans(dataset, samples, side, qa_spans) for side in ("pos", "neg")}


def main() -> None:
    parser = argparse.ArgumentParser(description="Diff-in-means direction identification on dataset mixtures.")
    ...
    setup_logging("mixed_direction_identification", RESULTS_DIR / "mixed_directions")
    logger.info(f"models={models} | datasets={DATASETS} | layers={args.layers} | seeds={seeds}")

    device = tl_utils.get_device()
    for model_name in models:
        out_base = RESULTS_DIR / "mixed_directions" / safe_model_id(model_name)
            ...
            for layer in todo:
                ...
                for combo in COMBOS:
                    ...
                    for pos_name in POSITIONS:
                        ...
                        direction = diff_in_means(pos_stack, neg_stack, normalize=False)
                        save_direction(
                            out_base / name / f"seed_{seed}" / "context_only" / f"layer_{layer}" / pos_name,
                            direction, pos_stack, neg_stack,
