...
Usage:
    python -m src.experiments.direction_identification --automated
    python -m src.experiments.direction_identification --automated --force-recompute
...
def collect_side_acts(
    ...
    """Residual-stream activations for one side. `items` is [(text, char_span_or_None)].

    Returns (last [n, d_model], entity [m, d_model]) where m counts the resolved
    spans; either tensor is None if nothing was collected.
    """
    tok = model.tokenizer
    ...
    for i in tqdm(range(0, len(items), batch_size), desc=desc):
        ...
        with torch.no_grad():
            # stop_at_layer skips every block above `layer` and the unembed. Without it
            # the logits tensor ([B, L, d_vocab]) is built and thrown away: GiB-scale on
            # large-vocab models, which fragments the allocator on long rows.
        ...
        last_acts.extend(resid[:, -1, :].detach().cpu().unbind(0))
        for r, (text, span) in enumerate(chunk):
            ...
            padded_idx = L - orig_lens[r] + entity_tok_idx
            padded_idx = max(0, min(padded_idx, L - 1))
            ent_acts.append(resid[r, padded_idx, :].detach().cpu())

    last = torch.stack(last_acts) if last_acts else None
    ent = torch.stack(ent_acts) if ent_acts else None
    return last, ent
