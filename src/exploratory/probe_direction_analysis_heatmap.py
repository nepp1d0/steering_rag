"""
Direction analysis heatmaps for the LOGISTIC-REGRESSION PROBE directions.

Same design as direction_analysis_heatmap.py (see that module for the full rationale),
reading the probe tree instead of the diff-in-means tree:

  - top layers  <- results/probes/top_retrieval_evaluation/...
  - directions  <- results/probes/direction_identification/...
  - outputs     -> results/probes/direction_analysis_heatmap/...

For EACH model we produce TWO heatmaps (3 rows x 2 cols), for BOTH positions
(last_pos, entity_pos):
...
Note on the centered cosine (decision 1 below): the original justification was phrased
for the diff-in-means direction, which has the corpus common-mean component subtracted
out by construction. A logistic-regression probe weight is a different object, but the
centering step here (subtract the eval corpus mean from the doc embeddings before the
cosine) removes the shared baseline regardless of how the direction was obtained, so the
factual/non-factual signal is exposed the same way. The specific "92% balanced accuracy"
verification note from the diff-in-means script does not literally transfer to probes.

No GPU / no model loading — everything reads precomputed tensors from disk.

Usage:
    python src/exploratory/probe_direction_analysis_heatmap.py
    python src/exploratory/probe_direction_analysis_heatmap.py --models google__gemma-3-4b-it
"""
...
from utils import RESULTS_DIR, load_normalized, logger, setup_logging
...
# Probe roots: parallel tree so nothing collides with the diff-in-means analysis.
TOP_ROOT = RESULTS_DIR / "probes" / "top_retrieval_evaluation"
DIRECTIONS_ROOT = RESULTS_DIR / "probes" / "direction_identification"
OUT_ROOT = RESULTS_DIR / "probes" / "direction_analysis_heatmap"
...
def discover_top_layer(model: str, eval_ds: str, direction_ds: str, position: str) -> int:
    ...
    path = (TOP_ROOT / model / eval_ds / direction_ds
...
def compute_cell(model: str, eval_ds: str, direction_ds: str, layer: int, position: str) -> tuple[float, float, float, float]:
    ...
    group = TOP_ROOT / model / eval_ds / direction_ds / "unnormalized"
    ...
    for s in SEEDS:
        ...
        # Precomputed probe direction (loaded, not recomputed).
        v = torch.load(
            DIRECTIONS_ROOT / model / direction_ds
...
def main() -> None:
    ...
    setup_logging("probe_direction_analysis_heatmap", OUT_ROOT)
